...
| 단계 | 항목 | 상태 |
|------|------|------|
| **1차 — MVP** | 인증, 채팅 UI, 세션 관리, 대화 히스토리, 파일, 감사 로그 | 완료 |
| **2차 — 버그 수정** | `--verbose` / `--include-partial-messages` 플래그 추가, 스트리밍 파싱 | 완료 |
| **3차 — 리치 렌더링** | 마크다운, 코드 구문 강조, HTML/SVG 프리뷰, 머메이드, 차트 | 진행 예정 |
| **4차 — 운영/관리** | 보안 강화, Docker, 감사 로그 UI, 사용자 관리, 세션 설정 | 미구현 |
| **5차 — SSH 모드** | 원격 서버 접속 | 미구현 |

...
| 패널 토글 | 사이드바 '파일 탐색기' 버튼으로 패널 열기/닫기 | 완료 |
| 리사이즈 | 드래그 핸들로 패널 너비 조절 (200~600px) | 완료 |
| 새로고침/접기 | 전체 트리 새로고침, 모든 폴더 접기 버튼 | 완료 |
| 파일 업로드 | 선택된 폴더 경로에 파일 업로드 | 완료 |

#### 변경된 파일

| 파일 | 변경 내용 |
|------|----------|
| `client/src/stores/explorerStore.js` | Explorer Zustand 스토어 (트리 상태, 확장/접기, 로딩, 리사이즈) |
| `client/src/components/Explorer/ExplorerPanel.jsx` | 오른쪽 패널 (헤더, 리사이즈 핸들, 액션 버튼) |
| `client/src/components/Explorer/ExplorerTree.jsx` | 재귀적 트리 뷰 (폴더/파일 아이콘, 확장 화살표, 스피너) |
| `client/src/components/Explorer/Explorer.css` | VS Code 스타일 CSS (트리 노드, 리사이즈, 스크롤바) |

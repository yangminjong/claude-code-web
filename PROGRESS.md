...
│   │   ├── stores/                 # Zustand (authStore, sessionStore, themeStore, cliSessionStore)
│   │   ├── App.jsx
│   │   ├── main.jsx
│   │   └── index.css
│   ├── index.html
...
| 단계 | 항목 | 상태 |
|------|------|------|
| **1차 — MVP** | 인증, 채팅 UI, 세션 관리, 대화 히스토리, 파일, 감사 로그, 버그 수정 | 완료 |
| **2차 — SSH 모드** | SSH 프로필, SSHFS 마운트, 원격 폴더 탐색기, 경로 검증, HTTPS | **완료** |
| **2.5차 — UI 개선** | 프로필 아바타, Claude 로고 아이콘, 멀티 테마 (6종) | **완료** |
| **2.6차 — 세션 지속성** | 세션 자동 이름 (첫 메시지 기반), 세션 재개 (`--resume`), claude_session_id DB 영속화, 세션 메타데이터 (크기/Git/시간), 세션 완전 삭제 (Claude CLI 포함) | **완료** |
| **2.7차 — CLI 세션 탐색기** | 사이드바 탭 (내 세션/CLI 히스토리), 프로젝트별 그룹핑, 키워드 검색, 통계, CLI→웹 세션 연결 (adopt), CLI 세션 삭제, 새로고침 버튼 | **완료** |
| **3차 — 리치 렌더링** | 마크다운, 코드 구문 강조, HTML/SVG 프리뷰, 머메이드, 차트, 도구 사용 알림 | 미구현 |
| **4차 — 운영/관리** | 보안 강화, Docker, 감사 로그 UI, 사용자 관리, 세션 설정 | 미구현 |
